...
class PosttestController extends Controller
{
    ...
    public function index()
    {
        ...
        $attempt = PosttestAttempt::where('user_id', Auth::id())->first();

        // check prasyarat
        $check = $this->canAccessPosttest(auth()->user());
        if (!$check['allowed']) {
            return view('mahasiswa.posttest', [
                'posttest' => $posttest,
                'attempt' => $attempt,
                'canAccess' => false,
        ...
        $canAccess = true;
        return view('mahasiswa.posttest', compact('posttest', 'attempt', 'canAccess',));
    }

    // =========================
    // Halaman pengerjaan posttest
    // =========================
    public function do()
    {
        $posttest = Posttest::first();
        ...
        // check prasyarat
        $check = $this->canAccessPosttest($user);
        if (!$check['allowed']) {
            return redirect()->route('mahasiswa.posttest')
                ->with('error', 'Persyaratan untuk mengerjakan posttest tidak terpenuhi!');
        }   

        // Pastikan posttest ada
        if (!$posttest) {
            return redirect()->route('mahasiswa.posttest')
                ->withErrors(['msg' => 'Posttest belum disetting oleh dosen.']);
        }

        // Cek kalau user sudah pernah submit atau timeout
        $finishedAttempt = PosttestAttempt::where('user_id', $user->id)
            ->where('posttest_id', $posttest->id)
            ->whereIn('status', ['submitted', 'timeout'])
            ->first();

        if ($finishedAttempt) {
            return redirect()->route('mahasiswa.posttest')
                ->withErrors(['msg' => 'Anda sudah mengerjakan posttest, tidak bisa mengerjakan lagi.']);
        }

        // Ambil attempt in_progress kalau ada (penting untuk refresh)
        $attempt = PosttestAttempt::where('user_id', $user->id)
            ->where('posttest_id', $posttest->id)
        ...
        $endTime = Carbon::parse($attempt->start_time)
                    ->addMinutes((int) $posttest->duration_minutes);
        ...
        return view('mahasiswa.posttest_do', compact('questions', 'attempt', 'endTimestampMs', 'posttest'));
    }



    // =========================
    // Submit jawaban posttest
    // =========================
    public function submit(Request $request)
    {
        ...
        $posttest = Posttest::first();
        $attempt = PosttestAttempt::where('user_id', Auth::id())
            ->where('status', 'in_progress')
            ->first();

        if (!$attempt) {
            return redirect()->route('mahasiswa.posttest')
                ->withErrors(['msg' => 'Tidak ada posttest yang sedang berlangsung.']);
        }
        ...
        return redirect()->route('mahasiswa.posttest')
            ->with('success', 'Posttest berhasil disubmit.');
    }

    // =========================
    // Soal posttest (array statis)
    // =========================
        ...
        return [
            [
                ...
                'options' => [
                    ...
                    'd' => 'zPernafasan dada lebih banyak dilakukan saat tidur',
                ],
                'answer' => 'c',
            ],
            [
    ...
    // =========================
    // Prasyarat mengakses posttest
    // =========================

    private function canAccessPosttest($user)
    {
        $userId = $user->id;
        // Test user
        ...
        $setting = Posttest::first();
        if(!$setting){
            return [
                'allowed' => false,
                'reason' => 'Post-test belum tersedia!'
            ];
        ...
        if ($setting->start_date && $now->lt($setting->start_date)) {
            return [
                'allowed' => false,
                'reason' => 'Post-test belum dapat diakses. Mulai pada ' .
        ...
        if ($setting->end_date && $now->gt($setting->end_date)) {
            return [
                'allowed' => false,
                'reason' => 'Periode pegerjaan post-test telah berakhir pada ' .
        ...
        if(!$setting->isActive()){
            return [
                'allowed' => false,
                'reason' => "Post-test dapat diakses pada ".Carbon::parse($setting->start_date)->format('d M Y H:i')." - ".Carbon::parse($setting->end_date)->format('d M Y H:i')
            ];
            
        }
        // Cek apakah sudah isi istrumen persepsi
        $perception = PerceptionResponse::where('student_id', $user->id)->exists();
        if (!$perception) {
            return [
                'allowed' => false,
                'reason' => 'Anda harus mengisi instrumen persepsi sebelum mengerjakan posttest.',
            ];
        }
