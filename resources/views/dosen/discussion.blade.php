...
<div class="bg-white rounded-4 shadow-sm mt-3 p-4">
    ...
    <div class="mb-3 pt-3">
        <form action="{{ route('dosen.discussion.store', $topic->id) }}" method="POST">
            @csrf
            ...
            <div class="d-flex mb-4 p-3 rounded-4 shadow-sm bg-white hover-shadow-sm">
                ...
                <div class="flex-grow-1">
                    ...
                    <div class="mt-3 d-flex gap-4">
                        ...
                        @if(auth()->id() === $discussion->user_id)
                            <button class="btn btn-sm btn-link text-danger text-decoration-none p-0" onclick="deleteAction({
                                        url: '{{ route('dosen.discussion.delete', ['topic_id' => $topic->id, 'discussion_id' => $discussion->id]) }}',
                    ...
                    <div id="reply-form-{{ $discussion->id }}" class="d-none my-5">
                        <form action="{{ route('dosen.discussion.reply', [$topic->id, $discussion->id]) }}" method="POST">
                            @csrf
                            <div class="d-flex align-items-start">
                                <div class="me-2">
                                    <img src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->name) }}&background=977CFF&color=fff"
                                    ...
                                    <div class="flex-grow-1">
                                        ...
                                        <div class="mt-1">
                                            @if(auth()->id() === $reply->user_id)
                                                <button class="btn btn-sm btn-link text-danger text-decoration-none p-0" onclick="deleteAction({
                                                    url: '{{ route('dosen.discussion.delete', [$topic->id, $reply->id]) }}',
                                                    text: 'Apakah Anda yakin ingin menghapus diskusi ini?',
                                                })">
                                                <i class="iconsax" icon-name="trash"></i> Hapus</button>
                                            @endif
